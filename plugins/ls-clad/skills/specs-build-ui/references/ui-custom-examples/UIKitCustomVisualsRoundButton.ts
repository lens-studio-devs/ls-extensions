 ...
 */
@component
export class UIKitCustomVisualsRoundButton extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">UIKitCustomVisualsRoundButton – gradient style for a UIKit RoundButton</span><br/><span style="color: #94A3B8; font-size: 11px;">Replaces the default visual with a custom gradient. Attach to the same SceneObject as RoundButton.</span>')
  @ui.separator
  ...
  private _button: RoundButton = null
  private _preset: StylePreset
  private logger: Logger

  onAwake() {
    // No Canvas created here — this script decorates an existing RoundButton on
    // the same SceneObject. The panel root that hosts the button must already
    // have a Component.Canvas in SortingType.Hierarchy (UIKit default) so DFS
    // owns paint order. No `renderOrder` anywhere.
    this.logger = new Logger("UIKitCustomVisualsRoundButton", this.enableLogging || this.enableLoggingLifecycle, true)
    if (this.enableLoggingLifecycle) this.logger.debug("LIFECYCLE: onAwake()")

    this._button = this.sceneObject.getComponent(RoundButton.getTypeName()) as RoundButton
    if (!this._button) {
