...
Reuse authentication across scripts
...
export class StorageLoader extends BaseScriptComponent {
  ...
  async loadAssets() {
    ...
    if (this.imageDisplay) {
      ...
      if (!error && data) {
        const resource = DynamicResource.createWithBuffer(await data.bytes());
...
heartbeatIntervalMs is required
...
Use `SUPABASE_SERVICE_ROLE_KEY` on the server
