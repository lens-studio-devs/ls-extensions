...
Use `CameraModule.requestImage` for still capture
...
Folder organization in the bucket
...
Capture to memory, upload after
...
Use `onNewFrame` instead of `UpdateEvent`
...
Stay under the 250 KB Realtime limit
...
Composite mode requires a buffer delay
...
`onNewFrame` vs `UpdateEvent` for camera mode
...
Composite buffer delay is essential
