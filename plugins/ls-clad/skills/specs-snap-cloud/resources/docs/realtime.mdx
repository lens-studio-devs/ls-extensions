...
> The upstream page at [developers.snap.com](https://developers.snap.com/spectacles/about-spectacles-features/snap-cloud/realtime) shows screenshots of the Realtime tab button, channel join UI, broadcast message dialog, and a received-message log view that are referenced below.

### Subscribe to a Channel

Realtime operates by broadcasting and subscribing to channels. We'll start by configuring our Lens to listen to a channel called `test-channel`. To do this, add the following function to the script from [Getting Started](https://developers.snap.com/spectacles/about-spectacles-features/snap-cloud/getting-started):
...
This setting allows your Lens to receive automatic notifications whenever data in the table changes (inserts, updates, or deletes). For the following examples, you can use the `test-table` that was created in the [Databases](https://developers.snap.com/spectacles/about-spectacles-features/snap-cloud/databases) guide.
