...
```typescript
// BAD: scene.findObjectByName() does not exist on ScriptScene — use @input instead
// (Shown here as a reminder that this pattern is invalid, not just slow)
onUpdate() {
    // scene.findObjectByName('Target') — THIS METHOD DOES NOT EXIST
}
